...
print("Tabla BigQuery:", TABLE_ID)

"""3: Imports, clientes y utilidades"""
...
if not GEMINI_API_KEY:
    raise ValueError("Guarda tu API key en Colab Secrets con el nombre GEMINI_API_KEY.")

gemini_client = genai.Client(api_key=GEMINI_API_KEY)

def query_to_df(sql, parameters=None):
...
print("BigQuery y Gemini listos.")

table = client.get_table(TABLE_ID)

print("Tabla encontrada:", TABLE_ID)
...
class BigQueryYouTubeRetriever:
    ...
    def find_video(self, video_reference):
        ...
        return self._query(sql, [
            bigquery.ScalarQueryParameter("channel_id", "STRING", CHANNEL_ID),
            bigquery.ScalarQueryParameter("needle", "STRING", f"%{video_reference.lower()}%"),
        ])
    ...
    def find_topic_moments(self, topic, filters=None, order_by="views", limit=8):
        rows = self.search_videos(topic, filters=filters, order_by=order_by, limit=limit)
        terms = extract_search_terms(topic)
...
"""#**7: Gemini como interpretador y generador RAG**"""
    ...
    def answer(self, question):
        ...
        if intent == "farewell":
            context = {
                ...
                "resumen": "Se trabajó con un agente RAG conectado a BigQuery y Gemini 2.5 Flash para analizar videos, métricas, temas, transcripciones y recomendaciones."
            }
        ...
        if intent == "improvements":
            ...
            try:
                context["videos_peor_de_lo_esperado_ml"] = self.retriever.predict_video_performance(limit=5, order="underperforming")
        ...
        if intent == "famous_person_opinion":
            context = {
                "persona": person,
                "nota": "No es una opinión real de la persona; es una simulación analítica basada en su estilo público y datos del canal.",
                "perfil_canal": self.retriever.channel_profile(),
                "metricas_generales": self.retriever.analytics_summary(),
                "temas_mejor_interaccion": self.retriever.topic_performance(limit=5, order_by="engagement"),
        ...
        if intent == "topic_moments":
            ...
            context = {
                ...
                "tema": topic,
                "nota_minutos": "Los minutos son aproximados porque la transcripción no trae timestamps reales por frase.",
        ...
        # Fallback RAG: intenta buscar videos relacionados con la pregunta completa.
        context = {
...
retriever = BigQueryYouTubeRetriever(client)
agent = RAGYouTubeAgent(retriever)
...
print("Agente RAG con BigQuery + Gemini 2.5 Flash + BigQuery ML listo.")

# Ejecuta esta celda una vez si todavía no has creado el modelo.
# Puede tardar y requiere permisos para crear modelos en BigQuery.

retriever.create_ml_model()

while True:
    pregunta = input("Pregunta: ")

    if pregunta.strip().lower() in {"salir", "exit", "quit"}:
        print("Listo, terminamos por hoy.")
        break
